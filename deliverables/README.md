...
Pages: Cover, Design system, Components, Assets, Machine screens, Mobile app, Machine sections, Sections variants.
Both surfaces are built natively, every layer a frame, text or vector and not one screenshot.

## Files

There are two clips, one for each surface: the score reveal on the glass, and one whole turn on the phone.

| File | What it is |
|---|---|
| `reveal-live-1920x1080.mp4` | **The clip to watch first.** The score reveal on the live glass as designed now, about 10 s at 60 fps: the last seconds of Punch now and the strike, Reading the strike, New record as the name at the top flaps over, then the Big score. The glass stands beside short beat names, with a close-up that follows the beat. |
| `reveal-live-1080x3840.mp4` | The same clip at the panel's real size and aspect, for playing on the cabinet. Many players and laptops cannot decode a 3840 px tall frame, which is why the presentation cut exists. |
| `reveal-live-960.gif` | The presentation cut as a GIF, in case a player is not to hand. |
| `reveal-live-1920x1080-poster.jpg` | A still of the presentation cut. |
...
the glass and the phone draw. Every Web Animation and every video on the page is paused and seeked to the same
...
the glass reads the strike and Your hit opens by itself. The recorder only starts the scan, taps the link and brings
