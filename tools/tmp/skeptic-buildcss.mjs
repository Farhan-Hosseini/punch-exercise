...
ws.addEventListener('message', (e) => {
  ...
  else if (m.method === 'Network.responseReceived') { const u = m.params.response.url; if (/\.css(\?|$)/.test(u)) net.push(`${m.params.response.status} ${u} from=${m.params.response.headers['cache-control'] || '-'}`) }
})
...
const sheets = JSON.parse(await js(`JSON.stringify([...document.styleSheets].map(s => { let n = -1; try { n = s.cssRules.length } catch {} return { href: (s.href||'').replace(location.origin,''), rules: n } }))`))
const modes = {}
